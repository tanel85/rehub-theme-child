                ...
                <div class="dokan-panel dokan-panel-default">
                    ...
                    <div class="dokan-panel-body general-details">
                        ...
                        <?php if ( 'off' === $hide_customer_info && ( $order->get_formatted_billing_address() || $order->get_formatted_shipping_address() ) ) : ?>
                            <ul class="list-unstyled customer-details" style="border-bottom: 1px solid #EDEDED; margin-bottom: 3px; padding-bottom: 3px;">
                                <li>
                        ...
                        <ul class="list-unstyled customer-details" style="border-bottom: 1px solid #EDEDED; padding-bottom: 3px;">
                            <?php
                            $billing_delivery = get_post_meta( dokan_get_prop( $order, 'id' ), '_billing_delivery', true );
                            if ( !empty( $billing_delivery ) ) {
                                $billing_delivery_label = $billing_delivery == '1' ? __( 'Soovin kohale toomist', 'dokan-lite' ) : __( 'Tulen ise järele', 'dokan-lite' );
                                ?>
                                <li>
                                    <span><?php esc_html_e( 'Kohaletoomine:', 'dokan-lite' ); ?></span>
                                    <?php echo esc_html( $billing_delivery_label ); ?>
                                </li>
                            <?php } ?>
                            <?php
                            $billing_delivery_date = get_post_meta( dokan_get_prop( $order, 'id' ), '_billing_delivery_date', true );
                            if ( !empty( $billing_delivery_date ) ) {
                                ?>
                                <li>
                                    <span><?php esc_html_e( 'Kohaletoomise või järeletulemise aeg:', 'dokan-lite' ); ?></span>
                                    <span style="background-color: #dff0d8;">
                                        <?php echo esc_html( $billing_delivery_date ); ?>
                                    </span>
                                </li>
                            <?php } ?>
                            <?php
                            $billing_state = get_post_meta( dokan_get_prop( $order, 'id' ), '_billing_state', true );
                            if ( !empty( $billing_state ) ) {
                                ?>
                                <li>
                                    <span><?php esc_html_e( 'Kohaletoomise piirkond:', 'dokan-lite' ); ?></span>
                                    <?php echo esc_html( WC()->countries->get_states('EE')[$billing_state] ); ?>
                                </li>
                            <?php } ?>
                            <?php
                            $billing_address_1 = get_post_meta( dokan_get_prop( $order, 'id' ), '_billing_address_1', true );
                            if ( !empty( $billing_address_1 ) ) {
                                ?>
                                <li>
                                    <span><?php esc_html_e( 'Kohaletoomise aadress:', 'dokan-lite' ); ?></span>
                                    <?php echo esc_html( $billing_address_1 ); ?>
                                </li>
                            <?php } ?>
                            <?php
                            $number_of_people = get_post_meta( dokan_get_prop( $order, 'id' ), '_billing_number_of_people', true );
                            if ( !empty( $number_of_people ) ) {
                                ?>
                                <li>
                                    <span><?php esc_html_e( 'Inimeste arv:', 'dokan-lite' ); ?></span>
                                    <?php echo esc_html( $number_of_people ); ?>
                                </li>
                            <?php } ?>
                        ...
                        <?php
                        $customer_note = get_post_meta( dokan_get_prop( $order, 'id' ), '_billing_order_comments', true );
